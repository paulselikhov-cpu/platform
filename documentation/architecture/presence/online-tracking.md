...
При целевом онлайне 100k+ (см.
[`../realtime/scale-targets-and-redis.md`](../realtime/scale-targets-and-redis.md))
хранение presence в PostgreSQL становится узким местом: каждая смена комнаты —
запись в БД. Целевое решение — перенести presence и счётчики в Redis, сохранив
контракт для клиента без изменений (те же WS-топики и REST-эндпоинты). До этапа R3
текущая схема (PostgreSQL) остаётся действующей.
