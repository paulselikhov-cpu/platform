...
| `PollCloseCheckHandler` | `scheduler/PollCloseCheckHandler.java` | Закрытие просроченных выборов через `SchedulerGateway` (реестр `ScheduledCheck`) |
...
| Компонент | Файл | Назначение |
|-----------|------|------------|
| `ElectionResultHandlerAdapter` | `handler/ElectionResultHandlerAdapter.java` | Назначение победителя на должность губернатора |
| `PollResultDispatcher` | `handler/PollResultDispatcher.java` | Диспетчер общего обработчика закрытия голосования |
...
`election.service`, `notification.service` (+live WS), `location-users.service`; модели `models/poll/election.interface.ts`, `models/notification/*`, `models/system-role/*`.
