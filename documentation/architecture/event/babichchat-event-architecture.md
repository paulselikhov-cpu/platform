...
> Дополнение к `vision/concept.md` и `system-model/mvp-ai-brief.md`. Отвечает на
...
Это прямое продолжение вашего принципа `RewardService(reason: ...)` —
...
Это тот же сценарий (Иванов проверяет `pashaSelikhov`), но показан
...
то самое разделение, о котором вы пишете между слоями («Слой 3 → Слой 4» — см. scenarios/2. the-first-election.md),
...
Раз уж концепция задаёт принцип "не смешивать роли даже в коде" —
...
| Задача | Паттерн | Где в коде |
|---|---|---|
| Начисления/списания | Единый сервис + explicit reason enum | `RewardService` + `RewardReason` |
| Заявки любого типа | Strategy + диспетчер по enum | `ApplicationHandler` (концепция 0.6) |
| Голосования любого типа | Strategy + диспетчер | `PollResultHandler` + `PollResultDispatcher` |
...
| Time-based переходы (закрытие Poll и т.п.) | Единый `SchedulerGateway` + реестр `ScheduledCheck` вместо разрозненных `@Scheduled` | `core/scheduler/` |
| Realtime доставка | STOMP over WebSocket, один коннект на чат+уведомления | Не описано явно — добавка |
| Кулдауны/энергия/квоты | Redis TTL | `concept.md` §9 — пока план (см. `realtime/scale-targets-and-redis.md`) |
