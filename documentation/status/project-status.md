...
2026-09-15 (Результат заявки адресно и с контекстом: Application.userId+characterId на Application; ApplicationResolvedEvent несёт characterId/applicationType/status (строки — границы модулей); NotificationEventListener уведомляет конкретного персонажа и кладёт в Notification.data applicationType/applicationStatus (NotificationContextData, key=value); NotificationResponse отдаёт data в WS/REST; фронт: INotificationContextData типизирован (ApplicationType/ApplicationStatus), CurrentChatUserService подписан на /user/queue/character-updated → refresh() профиля без F5; новая заметка architecture/reactive-character-updates.md (переписана из плана в факты))
...
| RoomView (бывш. LocationView) — вью комнаты: режимы отображения (views) кнопками внизу, у приёмной «Чат / Личный кабинет сотрудника / Услуги мэрии» (видимость по авторизации), у кабинета губернатора «Чат / Панель управления районом» (только губернатор); нет доп. режимов — кнопок нет; фича живёт в room-view (сессия сотрудника переживает переключение); лента вынесена в app-chat-area-messages; shared back-button | ✅ Выполнено (architecture/chat/room-feature.md) |
...
| Приёмная: «Гражданские заявки» (сотрудник мэрии) — меню со списком заявок, фильтр по типу (вкл. REGISTER_PASSPORT), кнопки Одобрить(зелёная)/Отклонить(красная); backend GET /api/applications/all с именем заявителя (ApplicationView) | ✅ Выполнено |
