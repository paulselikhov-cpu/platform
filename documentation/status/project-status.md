...
### 2.5 Сценарии проходимости

Статус игровых сценариев. Детальное описание каждого — в `documentation/scenarios/`.

| Сценарий | Статус | Что покрывает |
|----------|--------|---------------|
| №1. Первый вход в дефолтный район | ✅ Реализовано | Регистрация → главное меню → выбор района → вход в системные локации → просмотр комнат → чат |
| №2. Первые выборы | ❌ Не реализовано | Выборы губернатора на Площади Ленина |

**Реализация сценария №1 включает:**
- Компонент `MainMenu` (роут `/main-menu`) — приветственная заставка после регистрации/логина
- Компонент `DistrictSelect` (роут `/district-select`) — выбор района из списка
- Кликабельность системных локаций в сайдбаре с эмитом `systemLocationSelected`
- `ILocation` адаптирован для системных локаций (поля `template`, `inviteCode`, `isPublic` — опциональны)
- `LocationView` работает как с пользовательскими, так и с системными локациями (бейдж «Системная»)
- Иконки для каждого типа системной локации
...
### Навигация и социальное взаимодействие (сценарий №1)
```
Регистрация → Главное меню → Выбор района (Центральный)
  → Чат: сайдбар с системными локациями района
  → Клик на любую системную локацию (Мэрия, Площадь Ленина и т.д.)
    → Список комнат → выбор комнаты → чат
```
Пользователь может свободно перемещаться по системным локациям района,
общаться в их комнатах. Это базовый слой взаимодействия с игровым миром.
...
### При завершении крупного модуля (группировки, полиция, банк, рынок и т.д.):
1. Перенеси соответствующую строку из раздела 3 в раздел 2 (с кратким
   описанием реализованных сущностей/эндпоинтов)
2. Обнови раздел 4 (игровой цикл) — что теперь проходимо дальше
3. Актуализируй раздел 5 (следующий шаг)

### При изменении статуса сценария (синхронизация со `scenarios/*.md`):
1. Обнови статус шагов в самом файле сценария (`documentation/scenarios/*.md`)
2. Обнови таблицу сценариев в разделе 2.5 этого файла
3. Если изменился пользовательский путь — обнови раздел 4
4. Если доработка относится к шагу MVP — обнови раздел 3

Подробные правила синхронизации — в `documentation/.clinerules`, раздел
«Синхронизация сценариев и project-status.md».
