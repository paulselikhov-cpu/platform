...
ui:
По завершению выборов в shared/components/modals/district-info-modal.ts обновляется информация о том кто теперь мэр
Уведомления о начале и результатах выборов показываются в shared/components/modals/notification-info-modal.ts

**Требования к уровню:** Кнопку может запустить пользователь только с N-го уровня (PersonLevel, HP-прокачка). По умолчанию 1lvl. Проверку уровня делаем на бэке - в метод передаем только chat-user.id - если уровень не достаточен - возвращаем ошибку и обрабатываем её уведомлением. Уведомления будут справа вверху - сделаешь пока простой универсальный компонент уведомлений в shared/components
